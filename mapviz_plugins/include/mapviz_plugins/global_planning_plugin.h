...
#ifndef MAPVIZ_PLUGINS_GLOBAL_PLANNING_PLUGIN_H_
#define MAPVIZ_PLUGINS_GLOBAL_PLANNING_PLUGIN_H_
...
#include "ui_global_planning_config.h"

namespace mapviz_plugins
{
class GlobalPlanningPlugin : public mapviz_plugins::PointDrawingPlugin
{
  Q_OBJECT

public:
  GlobalPlanningPlugin();
  virtual ~GlobalPlanningPlugin();
...
private:
  Ui::global_planning_config ui_;
  QWidget* config_widget_;

  std::string topic_;
  ros::NodeHandle nh;

  ros::Subscriber waypoints_;
  ros::Subscriber bestpos_sub_;

  bool has_message_;
  // StampedPoint stamped_point;
  StampedPoint stamped_point_point;
  swri_transform_util::LocalXyWgs84Util local_xy_util_;

  void WaypointsCallback(const std_msgs::Float64MultiArrayConstPtr& array);
  void WaypointsStepsCallback(const std_msgs::Float64MultiArrayConstPtr& array);
  void WaypointsPrevNextCallback(const std_msgs::Float64MultiArrayConstPtr& array);
  void bestposCallback(const novatel_gps_msgs::NovatelPositionPtr& bestpos);
};
}

#endif  // MAPVIZ_PLUGINS_GLOBAL_PLANNING_PLUGIN_H_
