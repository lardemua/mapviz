...
PLUGINLIB_EXPORT_CLASS(mapviz_plugins::BestposPlugin, mapviz::MapvizPlugin)

namespace mapviz_plugins
{
BestposPlugin::BestposPlugin() : config_widget_(new QWidget())
{
...
BestposPlugin::~BestposPlugin()
{
}

void BestposPlugin::SelectTopic()
{
  ros::master::TopicInfo topic = mapviz::SelectTopicDialog::selectTopic("novatel_gps_msgs/NovatelPosition");
...
void BestposPlugin::TopicEdited()
{
  ...
  if (topic != topic_)
  {
    ...
    if (!topic.empty())
    {
      bestpos_sub_ = node_.subscribe(topic_, 1, &BestposPlugin::BestposCallback, this);

      inspva_sub_ = nh.subscribe("inspva", 1, &BestposPlugin::InspvaCallback, this);
...
void BestposPlugin::BestposCallback(const novatel_gps_msgs::NovatelPositionPtr& bestpos)
{
  ...
  stamped_point.stamp = bestpos->header.stamp;
  stamped_point.source_frame = local_xy_util_.Frame();
  double x;
  double y;

  local_xy_util_.ToLocalXy(bestpos->lat, bestpos->lon, x, y);

  stamped_point.point = tf::Point(x, y, bestpos->height);
  lap_checked_ = ui_.show_laps->isChecked();
}

void BestposPlugin::InspvaCallback(const novatel_gps_msgs::InspvaPtr& imu_inspva)
{
...
void BestposPlugin::PrintError(const std::string& message)
{
  PrintErrorHelper(ui_.status, message);
}

void BestposPlugin::PrintInfo(const std::string& message)
{
  PrintInfoHelper(ui_.status, message);
}

void BestposPlugin::PrintWarning(const std::string& message)
{
  PrintWarningHelper(ui_.status, message);
}

QWidget* BestposPlugin::GetConfigWidget(QWidget* parent)
{
...
bool BestposPlugin::Initialize(QGLWidget* canvas)
{
  canvas_ = canvas;
...
void BestposPlugin::Draw(double x, double y, double scale)
{
  if (DrawPoints(scale))
  {
    PrintInfo("OK");
...
void BestposPlugin::LoadConfig(const YAML::Node& node, const std::string& path)
{
  if (node["topic"])
  {
    std::string topic;
...
void BestposPlugin::SaveConfig(YAML::Emitter& emitter, const std::string& path)
{
